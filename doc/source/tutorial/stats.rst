...
Let's look at some linear data first:
    ...
    >>> linx = np.array([-0.69198779, 0.18236784, -0.55349325, -0.29817661,
    ...
    >>> liny = np.array([-0.6908471, 0.16075167, -0.67384637, -0.34711818,
...
The simulation relationship can be plotted below:
    ...
    >>> mgc_plot(linx, liny, "Linear", only_viz=True)
...
optimal scale is shown on the map as a red "x"::
    ...
    >>> stat, pvalue, mgc_dict = multiscale_graphcorr(linx, liny)
    >>> print("MGC test statistic: ", round(stat, 1))
    MGC test statistic:  1.0
    >>> print("P-value: ", round(pvalue, 1))
    P-value:  0.0
    >>> mgc_plot(linx, liny, "Linear", mgc_dict, only_mgc=True)

.. plot:: tutorial/stats/plots/mgc_plot2.py
   :align: center
   :include-source: 0

The same can be done for nonlinear data sets. The following `nonlinx` and
`nonliny` arrays are derived from a nonlinear simulation:

    >>> nonlinx = np.array([-0.57765724, -2.9276214, -1.04262652, 1.2582997,
    ...
    >>> nonliny = np.array([0.51031563, 0.38642023, -0.51907824, -1.27170782,
...
The simulation relationship can be plotted below:
    ...
    >>> mgc_plot(nonlinx, nonliny, "Spiral", only_viz=True)
...
optimal scale is shown on the map as a red "x":
    ...
    >>> print("P-value: ", round(pvalue, 1))
    P-value:  0.0
    >>> mgc_plot(nonlinx, nonliny, "Spiral", mgc_dict, only_mgc=True)

.. plot:: tutorial/stats/plots/mgc_plot4.py
   :align: center
   :include-source: 0

Included is also a faster implementation of MGC (Fast MGC) that operates in
:math:`\mathcal{O} (n \log n)` instead of the :math:`\mathcal{O} (n^2 \log n)`
time that MGC operates in. It does this by estimating the null distribution by
...
.. plot:: tutorial/stats/plots/mgc_plot5.py
   :align: center
   :include-source: 0
...
.. plot:: tutorial/stats/plots/mgc_plot6.py
   :align: center
   :include-source: 0
